...
Instead of just a score, it returns the exact sentence that hallucinated and what the context actually said. It's completely framework-agnostic—just pass it the `page_content` from your LangChain documents and the string output of your chain. It also supports local models via Ollama out of the box if you don't want to use OpenAI.
...
Hey folks! I've been building RAG with LlamaIndex and got annoyed trying to debug my pipelines when eval scores dropped. So I built **rageval** — an evaluation library that returns the specific *evidence* that caused a score to drop, not just a number. 
...
Hey everyone, I just open-sourced **rageval** — a RAG evaluation library built with local models in mind.
...
Check it out: https://github.com/sujalsonawane/rageval (`pip install rageval[ollama]`)
